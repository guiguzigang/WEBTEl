var Interface = function(name) {
	if(arguments.length != 2) throw new Error("创建的接口不符合标准，必须有两个参数，第二个参数就是接口的方法！");
	this.name = name;
	this.methods = [];
	var methods = arguments[1]; // 接口的方法，是一个数组
...
Interface.checkImplements = function(obj) {
	...
	for(var i=1; i<arguments.length; i++){
		...
		for(var j=0; j<cmethods.length; j++){
			if(!obj[cmethods[j]] || typeof obj[cmethods[j]] !== "function") throw new Error("必须实现：" + intObj.name + "的" + cmethods[j] + "这个方法！");
		}
	}
};